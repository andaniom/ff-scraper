...
def getEventsCalendarToday():
    ...
    day = int(
        format(int(day), "02"))  # Places 0's in front of the day if it is single digit day, for days 1-9 of the month
    ...
    eventProcess(events, url, day_of_week, event_date)

    print('Successfully retrieved all data')
    return True


def getEventsCalendar(start_date, end_date):
    global event_time_holder
    ...
    if start_date == end_date:
        ...
        getEventsCalendar(scrape_next_day, end_date)


def eventProcess(events, url, day_of_week, event_date):
    global event_time_holder
    for news in events:
        try:
            status = 'black'
            ...
            if impact == 'high':
                ...
                cursor.execute(sqlite_select_with_param, (parentId + status, status))

                result = cursor.fetchone()
                if result is None:
                    dateStr = event_date + " " + convert24(event_time_holder).strip()
                    ...
                    if status == 'black':
                        ...
                        if minutes_diff < 15:
                            print("send message")
                            send_msg(url, parentId, status, day_of_week, dateStr, curr, event, actual, forecast,
...
def send_msg(url, parent_id, status, day_of_week, date_str, curr, event, actual, forecast, previous, datetime_object,
             impact):
...
def LoadUpProxies():
    ...
    items = table.find_next('tbody').find_all('tr')

    for item in items:
        try:
            if item.select('td')[4].get_text() == "elite proxy" and item.select('td')[6].get_text() == "yes":
                proxies.append(item.select('td')[0].get_text() + ":" + item.select('td')[1].get_text())
...
if __name__ == "__main__":
    ...
    tele_auth_token = config['main']['tele_auth_token']
    tel_group_id = config['main']['tel_group_id']
    if tele_auth_token is None or tel_group_id is None:
        print("Missing Param")
        sys.exit(2)
    event_time_holder = ''
    ...
    while True:
        try:
            ...
            # randomProxy = proxies[rnd]
            # print(randomProxy)
            # proxy = {
            #     "http": f"http://{randomProxy}",
            #     "https": f"https://{randomProxy}",
            # }
            messages = getEventsCalendarToday()
            sleep(300)
        except requests.exceptions.ProxyError as err:
